...
async function compileContract() {
  ...
  try {
    ...
    execSync(
      "npx solcjs --bin --abi --include-path node_modules/ --base-path . -o ./build --optimize --optimize-runs 200 contracts/WeatherOracle.sol",
      {
...
async function deployToNetwork(networkName) {
  ...
  if (existingDeployment) {
    ...
    const featureFilePath = path.join(
      __dirname,
      "../oracle/features/WeatherOracle.js"
    );
    ...
    console.log(
      `Updated WeatherOracle feature with existing contract address: ${existingDeployment.address}`
    );
  ...
  console.log(
    `Deployment information saved to ${deploymentDir}/WeatherOracle.json`
  );
  ...
  fs.writeFileSync(featureFilePath, updatedContent);
  console.log(
    `Updated WeatherOracle feature with new contract address: ${contractAddress}`
  );
